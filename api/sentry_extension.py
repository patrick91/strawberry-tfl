...
class SentryExtension(Extension):
    ...
    async def resolve(self, _next, root, info, *args, **kwargs):
        ...
        with self.transaction.start_child(
            ...
            transaction.set_tag("graphql.parent_type", info.parent_type.name)
            transaction.set_tag("graphql.field_path", field_path)
            transaction.set_tag("graphql.path", ".".join(map(str, info.path.as_list())))
